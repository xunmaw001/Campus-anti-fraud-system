...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussanquanzhishiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussanquanzhishiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussanquanzhishi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussanquanzhishi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussanquanzhishi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussanquanzhishi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussanquanzhishi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussanquanzhishi', this.detail);
				this.$utils.msg('审核成功');
