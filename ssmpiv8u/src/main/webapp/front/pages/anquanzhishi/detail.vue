...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discussanquanzhishiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussanquanzhishiCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','anquanzhishi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','anquanzhishi');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('anquanzhishi', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussanquanzhishi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussanquanzhishi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('anquanzhishi', this.detail);
				this.$utils.msg('审核成功');
